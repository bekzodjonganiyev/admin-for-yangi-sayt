  ...
  <div class="flex flex-col">
    ...
    <div class="flex m-2 pl-14 flex-wrap">
      <a
        ...
        target="_blank"
        :href="'http://backend.tkti.uz' + '/' + file.photo"
        :key="index"
      >
        <img
          class="m-2 w-20 h-20 rounded-full"
          :src="'http://backend.tkti.uz' + '/' + file.photo"
          width="200"
          height="auto"
        />
      </a>
      ...
      UploadService.upload(
        ...
        .then((files) => {
          UploadService.getFiles(this.getUrl).then((response) => {
            this.fileInfos = response.data.data.filter(
              (a) => a.title_uz === this.stringData.title_uz
            );
          });
        })
        .catch(() => {
