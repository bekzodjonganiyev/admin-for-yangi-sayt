  ...
  <Layout>
    ...
    <div class="my-5">
          ...
          <table class="w-full">
            <thead class="bg-gray-100">
              <tr>
                ...
                <th class="w-32 text-zinc-400 p-3 text-sm font-semibold tracking-wide text-left">Kafedra yo'nalishlari
                </th>
                <th class="w-32 text-zinc-400 p-3 text-sm font-semibold tracking-wide text-left">Amallar</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-100">
              <tr v-for="(kafedra, index) in kafedras" :key="index" class="bg-white">
                <td class="p-3 text-sm text-gray-700 whitespce-nowrap">{{ kafedra.title_uz }}</td>
      ...
      <div class="flex justify-between items-center mt-16">
        <h1 class="text-xl  ">Kafedra Xodimlari</h1>
        <select class="w-96 p-2" ref="kafedra" @change="filterByKafdedra">
          <option value="all">Kafedrani tanlang</option>
          <option v-for="name in kafedras" :value="name._id" :key="name._id">{{ name.title_uz }}</option>
        </select>
      </div>
      <div class="h-full w-full mt-4">
        <div class="overflow-auto p-5 bg-white rounded-lg shadow hidden md:block">
          <table class="w-full">
            <thead class="bg-gray-100">
              <tr>
                ...
                <th class="w-32 text-zinc-400 p-3 text-sm font-semibold tracking-wide text-left">Lavozimi</th>
            ...
            <tbody class="divide-y divide-gray-100">
              <tr v-for="(xodim, index) in filteredArray" :key="index" class="bg-white">
                ...
                <td class=" whitespce-nowrap">
                  ...
                  <button type="button" @click="deleteKafXodim(xodim._id)"
                    class="bg-red-400 py-1 px-2 rounded">Delete</button>
                </td>
              </tr>
            </tbody>
  ...
  methods: {
    getData() {
      http.get("kafedra_data/all")
        .then((res) => {
          this.kafedras = res.data.data;
          console.log(res.data.data)
        }).catch(error => {
          console.log(error);
        })

      http.get("kafedra_hodim/all")
        .then((res) => {
    ...
    deleteData(id) {
      if (confirm('This element will be delete!!! ⚠⚠⚠')) {
        http.delete(`/kafedra_data/${id}`).then(() => {
          this.getData()
        })
      }
    },
    ...
    deleteKafXodim(id) {
      if (confirm('This element will be delete!!! ⚠⚠⚠')) {
        http.delete(`/kafedra_hodim/${id}`).then(() => {
          this.getData()
        })
      }
    },
