    ...
    <div class="my-5">
        ...
        >
          <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-100">
              <tr
                v-for="(kafedra, index) in kafedras"
                :key="index"
                class="bg-white"
              >
                ...
                <td class="p-3 text-sm text-gray-700 whitespce-nowrap">
                  <p
                    v-for="(kafedra, index) in kafedra.yonalish_uz"
                    :key="index"
                  >
                    {{ kafedra }}
                  </p>
      ...
      <div class="flex justify-between items-center mt-16">
        ...
        <select class="w-96 p-2" ref="kafedra" @change="filterByKafdedra">
          ...
          <option v-for="name in kafedras" :value="name._id" :key="name._id">
            {{ name.title_uz }}
          </option>
        </select>
      </div>
      <div class="h-full w-full mt-4">
        ...
        >
          <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-100">
              <tr
                v-for="(xodim, index) in filteredArray"
                :key="index"
                class="bg-white"
              >
                <td class="p-3 text-sm text-gray-700 whitespce-nowrap">
                  <img
                    :src="defaultUrl + '/' + xodim.photo"
                    alt="xodim rasmi"
                    width="100"
                    height="auto"
                  />
                ...
                <td class="whitespce-nowrap">
                  ...
                  <button
                    type="button"
                    @click="deleteKafXodim(xodim._id)"
                    class="bg-red-400 py-1 px-2 rounded"
                  >
                    Delete
                  </button>
  ...
  methods: {
    ...
    filterByKafdedra() {
      ...
      if (id === "all") {
        ...
        this.filteredArray = this.employes.filter(
          (item) => item.kafedra_id === id
        );
      }
    },
  },
