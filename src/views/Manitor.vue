<template>
  <Layout>
    <div class="">
      ...
      <div class="bg-white py-7 px-4 rounded-lg shadow overflow-y-scroll">
          ...
          <tbody class="divide-y divide-gray-100">
            <tr v-for="item in storeItems" :key="item.id">
              <td class="p-3 text-sm text-gray-700 whitespce-nowrap">
                {{ item.item_barcode[0].id }}
              </td>
              <td class="p-3 text-sm text-gray-700 whitespce-nowrap">
                {{ item.item_barcode[0].store_name }}
              </td>
              <td class="p-3 text-sm text-gray-700 whitespce-nowrap">
                {{ item.item_barcode[0].item_name }}
              </td>
              <td class="p-3 text-sm text-gray-700 whitespce-nowrap">
                {{ item.item_barcode[0].item_price }}
              </td>
              <td class="p-3 text-sm text-gray-700 whitespce-nowrap">
                {{ item.item_barcode[0].qty }}
              </td>
            </tr>
          </tbody>
        </table>
...
export default {
  ...
  methods: {
    getPost() {
      http.get(`/item/get/`).then((res) => {
        this.storeItems = res.data.results;
      });
    },
    getPost2() {
      http.get(`/store-item/get/`).then((res) => {
